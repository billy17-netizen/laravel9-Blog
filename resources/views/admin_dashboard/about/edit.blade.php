@extends("admin_dashboard.layouts.app")

@section('style')
    <script src="https://cdn.tiny.cloud/1/aupasg6zdvahhi9lmu92jxp6s8memctg1cwq6ok71og3qrlt/tinymce/5/tinymce.min.js" referrerpolicy="origin"></script>
@endsection
@section("wrapper")
    <!--start page wrapper -->
    <div class="page-wrapper">
        <div class="page-content">
            <!--breadcrumb-->
            <div class="page-breadcrumb d-none d-sm-flex align-items-center mb-3">
                <div class="breadcrumb-title pe-3">About</div>
                <div class="ps-3">
                    <nav aria-label="breadcrumb">
                        <ol class="breadcrumb mb-0 p-0">
                            <li class="breadcrumb-item"><a href="{{route('admin.index')}}"><i class="bx bx-home-alt"></i></a>
                            </li>
                            <li class="breadcrumb-item active" aria-current="page">About Page</li>
                        </ol>
                    </nav>
                </div>
            </div>
            <div class="card">
                <div class="card-body p-4">
                    <h5 class="card-title">Edit About Page</h5>
                    <hr/>
                    <form action="{{route('admin.settings.update')}}" method="post" enctype="multipart/form-data" >
                        @csrf
                        <div class="form-body mt-4">
                            <div class="row">
                                <div class="col-lg-12">
                                    <div class="border border-3 p-4 rounded">
                                        <div class="mb-3">
                                            <label for="first_text" class="form-label">Top Text</label>
                                            <textarea class="form-control" id="first_text" name="about_first_text">{{$setting->about_first_text}}</textarea>
                                            @error('about_first_text')
                                            <span class="text-danger">{{$message}}</span>
                                            @enderror
                                        </div>

                                        <div class="mb-3">
                                            <label for="second_text" class="form-label">Second Text</label>
                                            <textarea  class="form-control" id="second_text" name="about_second_text" >{{$setting->about_second_text}}</textarea>
                                            @error('about_second_text')
                                            <span class="text-danger">{{$message}}</span>
                                            @enderror
                                        </div>

                                        <div class="row">
                                            <div class="col-md-8">
                                                <div class="mb-3">
                                                    <label for="input_image1" class="form-label"> First Image</label>
                                                    <input type="file" class="form-control" id="input_image1" name="about_first_image">
                                                    @error('about_first_image')
                                                    <span class="text-danger">{{$message}}</span>
                                                    @enderror
                                                </div>
                                            </div>
                                            <div class="col-md-4">
                                                <div class="user-image p-2">
                                                    <img class="img-fluid  img-thumbnail" src="{{asset('/storage/' . $setting->about_first_image)}}" alt="">
                                                </div>
                                            </div>
                                        </div>

                                        <div class="row">
                                            <div class="col-md-8">
                                                <div class="mb-3">
                                                    <label for="input_image2" class="form-label"> Second Image</label>
                                                    <input type="file" class="form-control" id="input_image2" name="about_second_image">
                                                    @error('about_second_image')
                                                    <span class="text-danger">{{$message}}</span>
                                                    @enderror
                                                </div>
                                            </div>
                                            <div class="col-md-4">
                                                <div class="user-image p-2">
                                                    <img class="img-fluid  img-thumbnail" src="{{asset('/storage/' .$setting->about_second_image)}}" alt="">
                                                </div>
                                            </div>
                                        </div>
                                        <div class="mb-3">
                                            <label for="our_mission" class="form-label">About our Mission</label>
                                            <textarea class="form-control" id="about_our_mission"
                                                      rows="3" name="about_our_mission">{{old('about_our_mission',$setting->about_our_mission)}}</textarea>
                                            @error('about_our_mission')
                                            <span class="text-danger">{{$message}}</span>
                                            @enderror
                                        </div>
                                        <div class="mb-3">
                                            <label for="our_vision" class="form-label">About our Vision</label>
                                            <textarea class="form-control" id="about_our_vision"
                                                      rows="3" name="about_our_vision">{{old('about_our_vision',$setting->about_our_vision)}}</textarea>
                                            @error('about_our_vision')
                                            <span class="text-danger">{{$message}}</span>
                                            @enderror
                                        </div>
                                        <div class="mb-3">
                                            <label for="services" class="form-label">About Services</label>
                                            <textarea class="form-control" id="about_services"
                                                      rows="3" name="about_services">{{old('about_services',$setting->about_services)}}</textarea>
                                            @error('services')
                                            <span class="text-danger">{{$message}}</span>
                                            @enderror
                                        </div>
                                            <button type="submit" class="btn btn-primary">Update</button>
                                    </div>
                                </div>
                                <!--end row-->
                            </div>
                        </div>
                    </form>
                </div>
            </div>
        </div>
    </div>
    <!--end page wrapper -->
@endsection

@section("script")
    <script>


        setTimeout(function(){
            $('.general-message').fadeOut();
        },5000);

        let initTinyMCE = (id) =>{
            tinymce.init({
                selector: '#'+id,
                plugins: 'advlist autolink link lists charmap print preview hr anchor pagebreak ',
                toolbar: 'insertfile undo redo | styleselect | bold italic | alignleft aligncenter alignright alignjustify | link image | tinycomments | casechange permanentpen | bullist numlist outdent indent | ',
                toolbar_mode: 'floating',
                tinycomments_mode: 'embedded',
                tinycomments_author: 'Author name',
                height: 300,

            });
        }
        initTinyMCE('about_our_mission');
        initTinyMCE('about_our_vision');
        initTinyMCE('about_services');
    </script>
@endsection
