                ...
                <div class="ps-3">
                    <nav aria-label="breadcrumb">
                        <ol class="breadcrumb mb-0 p-0">
                            ...
                            <li class="breadcrumb-item active" aria-current="page">{{$user->name}} User Posts</li>
                        </ol>
                    </nav>
                </div>
            </div>
                        ...
                        <table class="table mb-0">
                            ...
                            @foreach($user->posts as $post)
                                <tr>
                                    ...
                                    <td><div class="badge rounded-pill @if($post->status === 'published') ? {{'text-info bg-light-info'}} @elseif($post->status === 'draft') {{'text-warning bg-light-warning'}} @else {{'text-danger bg-light-danger'}} @endif p-2 text-uppercase px-3"><i class='bx bxs-circle align-middle me-1'></i>{{$post->status}}</div></td>
                                    <td>{{$post->views}}</td>
                                    <td>
